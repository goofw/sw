function(module, exports) {
    ...
    exports.encode = function(plain) {
        var buff, quintets, i = 0, j = 0, shiftIndex = 0, digit = 0, encoded = new Buffer(8 * (buff = plain, 
        quintets = Math.floor(buff.length / 5), buff.length % 5 == 0 ? quintets : quintets + 1));
        for (Buffer.isBuffer(plain) || (plain = new Buffer(plain)); i < plain.length; ) {
            var current = plain[i];
