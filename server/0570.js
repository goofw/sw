function(module, exports, __webpack_require__) {
    ...
    encode.bytes = -1, encode._floatConversionDetected = !1, encode._encode = function(buffers, data) {
        if (Buffer.isBuffer(data)) return buffers.push(Buffer.from(data.length + ":")), 
        void buffers.push(data);
        if (null != data) switch (typeof data) {
          case "string":
    ...
    encode.buffer = function(buffers, data) {
        ...
        buffers.push(buffD);
        for (var k, j = 0, keys = Object.keys(data).sort(), kl = keys.length; j < kl; j++) null != data[k = keys[j]] && (encode.buffer(buffers, k), 
