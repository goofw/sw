function(module, exports) {
    var I = "i".charCodeAt(0), L = "l".charCodeAt(0), E = "e".charCodeAt(0), D = "d".charCodeAt(0);
    ":".charCodeAt(0);
    var BdecodeSMachine = function(cb, cb_list, cb_dict, cb_end) {
        ...
        strLen = 0, this.parse = function(buffer, encoding) {
            encoding = encoding || "UTF-8", "string" == typeof buffer && (buffer = new Buffer(buffer, encoding));
            for (var pos = 0; pos != buffer.length; ++pos) switch (state) {
              ...
              case 3:
                if (buffer[pos] != ":".charCodeAt(0)) throw new Error("not a colon at:" + pos.toString(16));
              ...
              case 4:
                if (state = 5, buffer[pos] == "-".charCodeAt(0)) {
                    neg = !0;
                    break;
                }
              ...
              case 5:
                if (integer(buffer[pos])) _int *= 10, _int += buffer[pos] - 48; else {
                    if (buffer[pos] != "e".charCodeAt(0)) throw new Error("not part of int at:" + pos.toString(16));
                    cb(neg ? 0 - _int : _int), state = 0;
                }
            }
        };
        var integer = function(value) {
            return "number" == typeof value && between(value, 48, 57);
        ...
        switch (typeof obj) {
          ...
          case "object":
            return obj instanceof Array ? (function(obj) {
                ...
                return assemble(obj, "d", (function(obj, pos) {
                    for (var p in obj) {
                        var key = Bencode(p), val = Bencode(obj[p]);
                        ensure(key.length + val.length, pos), key.copy(buffer, pos, 0), pos += key.length, 
                        val.copy(buffer, pos, 0), pos += val.length;
                    }
                    return pos;
                }));
            })(obj);
    ...
    exports.encode = Bencode, exports.decoder = Bdecode, exports.decode = function(buffer, encoding) {
